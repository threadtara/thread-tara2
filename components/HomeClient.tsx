...
// components/HomeClient.tsx — all client-side logic lives here
// Fixes applied:
//   1. "use client" moved out of page.tsx so metadata works
//   2. h2 → h1 for hero heading (only one h1 per page)
//   3. Background image alt="" (decorative image)
//   4. Logo alt is now descriptive
//   5. Three duplicate <style jsx global> blocks merged into one
//   6. JsonLd stays here (client render is fine for JSON-LD)
...
export default function HomeClient() {
  ...
  return (
    <main className="min-h-screen">
      ...
      {/* ── All animations consolidated into ONE style block ── */}
      <style jsx global>{`
        ...
        .animate-flag-flow    { animation: flag-flow 12s ease-in-out infinite; }
      `}</style>
      ...
      <section className="relative flex h-[85vh] items-center justify-center overflow-hidden">
        ...
        {/* Background image — decorative, so alt="" */}
        ...
        <div className="relative z-10 px-6 text-center flex flex-col items-center">
          ...
          <div className="relative mx-auto flex justify-center overflow-hidden">
            <Image
              ...
              width={420}
              height={420}
              // FIX 4: descriptive alt instead of just "Logo"
              alt="Thread Tara — Premium Fabric Store, Lajpat Nagar Delhi"
              className="mx-auto animate-fade-in-up transition-all duration-1000"
              style={{ filter: "drop-shadow(0 4px 15px rgba(212,175,55,0.3))" }}
            />
          </div>

          {/* FIX 2: h2 → h1 — only one h1 allowed per page */}
          <h1
            ref={textRef}
            className="mb-8 mt-8 font-serif text-3xl text-white md:text-5xl font-light leading-tight"
          >
            Premium Textiles For Every Vision
          </h1>

          <div className="mx-auto h-1 w-32 bg-fabric-gold mb-8 rounded-full" />
      ...
      {/* Fabric collection — h2 is correct here (below the h1) */}
...
// ── Silk thread overlay — kept as local component ────────────
const ThreadOverlay = () => (
